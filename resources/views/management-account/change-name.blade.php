        ...
        <div class="container-fluid container-md">
            <div class="col-auto col-sm-8 offset-sm-2 col-lg-6 offset-lg-3">
                ...
                <div class="p-4 border rounded-3">
                    <form id="myForm" action="/account-management/dashboard/account/edit-name" method="post"
                        novalidate>
                        @csrf
                        <!-- header form -->
                        <h1 class="mb-4 text-center text-secondary">Edit Name</h1>
                        <!-- end header form -->

                        <!-- disabled input -->
                        <div class="form-floating mb-3">
                            <input class="form-control opacity-50" name="old-name-display" id="old-name-input"
                                type="text" placeholder="old-name" value="{{ $name ?? "@name_is_not_set"}}"
                                disabled />
                            <label for="old-name-input">Old Name</label>
                            <input type="hidden" name="old-username" value="{{ $username ?? null }}">
                            <input type="hidden" name="old-name" value="{{ $name ?? null }}">
                            <div class="form-text">Currently your name registered on.</div>
                        </div>
                        ...
                        <div class="form-floating mb-3">
                            ...
                            <input class="form-control" name="new-name" id="new-name-input" type="text" value=""
                                placeholder="new-name" required />
                            <label for="new-name-input">New Name</label>

                            <!-- username error message -->
                            @isset($errorNewNameInput)
                                <div class="text-danger"> {{ $errorNewNameInput }} </div>
                            @endisset
                            <!-- end username error message -->

                            <div class="form-text">Example: Andrew Davis</div>
                        ...
                        <div class="form-floating mb-2">
                            <button type="submit" class="btn btn-outline-secondary w-100">Change Name</button>
                        </div>
